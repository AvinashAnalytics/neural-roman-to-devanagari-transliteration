...
"""
LSTM Training Script for Hindi Transliteration
"""
...
class LSTMTrainer:
    """LSTM Trainer with full config integration and assignment compliance"""
    ...
    def __init__(self, config_path: str = "config/config.yaml", use_wandb: bool = False):
        ...
        # Load test data with grouped references
        ...
        logger.info(f"   Bidirectional: {self.model.bidirectional}")
        logger.info(f"   Attention: {self.model.use_attention}")
        
        # FIXED: Use model-specific training config
        train_config = self.config['training']
        model_config = train_config.get('lstm_specific', {})
        ...
        # FIXED: Get optimizer params from lstm_specific
        ...
        if self.use_wandb:
            wandb.init(
                ...
                name=f"lstm-{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                config=self.config,
        ...
        # FIXED: Teacher forcing from lstm_specific section
        self.teacher_forcing_ratio = model_config.get('teacher_forcing_ratio', 0.5)
        self.teacher_forcing_decay = model_config.get('teacher_forcing_decay', 0.99)
        self.teacher_forcing_min = model_config.get('teacher_forcing_min', 0.1)
        ...
        logger.info(f"   Teacher forcing: {self.teacher_forcing_ratio:.2f} (decay: {self.teacher_forcing_decay:.3f})")
    ...
    def _create_scheduler(self):
        ...
        scheduler_type = model_config.get('scheduler_type', 'reduce_on_plateau').lower()
        ...
        if scheduler_type == 'reduce_on_plateau':
            ...
            return optim.lr_scheduler.StepLR(
                self.optimizer,
                step_size=model_config.get('lr_decay_steps', 5),
                gamma=model_config.get('lr_decay_factor', 0.5)
            )
        else:
            logger.warning(f"Unknown scheduler type: {scheduler_type}, using ReduceLROnPlateau")
            return optim.lr_scheduler.ReduceLROnPlateau(
                self.optimizer, mode='max', patience=3, factor=0.5
            )
    
    def _compute_src_lengths(self, src: torch.Tensor) -> torch.Tensor:
        """Compute actual lengths of source sequences (excluding padding)"""
        src_lengths = (src != self.data_manager.src_vocab.pad_idx).sum(dim=1)
        return src_lengths
    ...
    def train_epoch(self, epoch: int) -> float:
        ...
        # FIXED: Update teacher forcing ratio with exponential decay
        current_tf_ratio = max(
            self.teacher_forcing_min,
            self.teacher_forcing_ratio * (self.teacher_forcing_decay ** (epoch - 1))
        )
        
        progress_bar = tqdm(
            self.train_loader,
            desc=f'Epoch {epoch}/{self.num_epochs} [TF={current_tf_ratio:.2f}]',
            ncols=120,
            leave=True
        )
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            src_lengths = self._compute_src_lengths(src)
            ...
            if self.use_amp:
                with autocast():
                    ...
                    # LSTM outputs full sequence predictions [B, L, V]
                    loss = self.criterion(
                        outputs.view(-1, self.model.tgt_vocab_size),
                        tgt.view(-1)
                    )
            ...
            if self.use_wandb and self.global_step % self.log_every_n_steps == 0:
                wandb.log({
                    ...
                    'train/teacher_forcing_ratio': current_tf_ratio,
    ...
    def validate(self) -> Tuple[float, Dict]:
        ...
        with torch.no_grad():
            for batch in tqdm(self.valid_loader, desc='Validating', ncols=100, leave=False):
                ...
                # Compute loss (no teacher forcing)
                ...
                pred_indices = self.model.generate(src, beam_size=1, src_lengths=src_lengths)
    ...
    def train(self):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            self.history['teacher_forcing_ratio'].append(current_tf)
            ...
            logger.info(f"  Teacher Forcing: {current_tf:.2f}")
        ...
        # Save history
        history_path = self.results_dir / 'lstm_training_history.json'
    ...
    def save_checkpoint(self, epoch: int, is_best: bool = False, metrics: Optional[Dict] = None):
        ...
        if self.keep_last_n_checkpoints > 0:
            checkpoints = sorted(
                self.checkpoint_dir.glob('lstm_epoch*.pt'),
    ...
    def compare_decoding_methods(self, sample_size: int = None):
        ...
        for beam_size in beam_sizes:
            ...
            with torch.no_grad():
                for batch in self.valid_loader:
                    ...
                    if beam_size == 1:
                        output = self.model.generate(src, beam_size=1, src_lengths=src_lengths)
                        ...
                        beam_output = self.model.generate(src, beam_size=beam_size, src_lengths=src_lengths)
                        for i in range(min(len(batch['src_text']), sample_size - sample_count)):
                            pred = self.data_manager.tgt_vocab.decode(
        ...
        comparison_path = self.results_dir / 'lstm_decoding_comparison.json'
    ...
    def test(self):
        ...
        best_checkpoint_path = self.checkpoint_dir / 'lstm_best.pt'
        ...
        for beam_size in beam_sizes:
            ...
            with torch.no_grad():
                for src_word in tqdm(self.test_sources, desc=f'Beam-{beam_size}', ncols=100):
                    ...
                    src_lengths = self._compute_src_lengths(src_tensor)
                    
                    if beam_size == 1:
                        output = self.model.generate(src_tensor, beam_size=1, src_lengths=src_lengths)
                        pred_text = self.data_manager.tgt_vocab.decode(
        ...
        # Save results
        results_path = self.results_dir / 'lstm_test_results.json'
        ...
        table_path = self.results_dir / 'lstm_comparison_table.md'
        with open(table_path, 'w', encoding='utf-8') as f:
            f.write("# LSTM Transliteration Results\n\n")
            f.write(table)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train LSTM for Hindi Transliteration')
    ...
    try:
        trainer = LSTMTrainer(config_path=args.config, use_wandb=args.wandb)
